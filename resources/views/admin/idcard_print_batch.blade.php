...
<style>
    ...
    @media print { 
        ...
        /* 2-up layout on print with reduced gap */
        .card-wrap{ width:50% !important; float:left; padding:6px; }
        /* Set printed card width to 40% for batch layout */
        .idcard{ width:85% !important; margin:0 auto; }
    }
    .card-wrap{ break-inside: avoid; page-break-inside: avoid; margin-bottom: 10px; }
    .idcard { border: 1px solid #e6e6e6; border-radius: 10px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,.06); background:#ffffff; width: 55%; margin: 0 auto; }
    ...
    .avatar{ position: relative; width: 110px; height:110px; border-radius: 999px; object-fit: cover; border: 4px solid #fff; margin-top: -50px; background:#fff; }
    ...
    .name { text-align:center; font-weight: 700; font-size: 18px; margin: 6px 0 2px; }
    .subtitle { text-align:center; color:#6b7280; font-size: 12px; margin-bottom: 8px; }
    .kv { display:grid; grid-template-columns: 70px 1fr; gap: 5px 8px; font-size: 12px; padding: 6px 8px; background: #f9fafb; border-radius:8px; }
    .kv .k{ color:#6b7280; }
    .kv .v{ font-weight:600; }
    .bottom-row{ display:flex; justify-content:space-between; align-items:center; margin-top:8px; }
    .qr{ width:70px; height:70px; background:#fff; border:1px solid #e5e7eb; border-radius:6px; display:flex; align-items:center; justify-content:center; }
    .footer-note{ text-align:right; color:#6b7280; font-size:11px; }
    /* Match footer parallax image size to QR size (70x70) */
    .footer-img{ width:90px; height:70px; padding:0.2rem; object-fit:fill; border:1px solid #e5e7eb; border-radius:6px; }
</style>
<div class="row mb-3">
    <div class="col-12 d-flex justify-content-between align-items-center">
        <div class="d-flex align-items-center gap-3">
            <h5 class="mb-0">Batch: {{ $batch }}</h5>
            <span class="badge bg-secondary">Total Cards: {{ count($students) }}</span>
        </div>
        <button onclick="window.print()" class="btn btn-primary no-print"><i class="fa-solid fa-print"></i> Print</button>
    </div>
</div>
<div class="row justify-content-center">
@foreach($students as $student)
    @php $avatar = !empty($student->avatar) ? asset('public/upload/student/'.$student->avatar) : asset('public/admin/velzon/html/default/assets/images/users/avatar-1.jpg'); @endphp
    <div class="col-6 col-md-6 d-flex justify-content-center card-wrap">
        <div class="idcard my-2">
                ...
                <div class="bottom-row">
                    ...
                    <div class="qr">
                        @if(class_exists('SimpleSoftwareIO\\QrCode\\Facade\\QrCode'))
                            {!! SimpleSoftwareIO\QrCode\Facade\QrCode::size(70)->format('svg')->margin(0)->generate($qrText) !!}
                        @else
                            @php $qrData = urlencode($qrText); @endphp
                            <img src="https://api.qrserver.com/v1/create-qr-code/?size=70x70&data={{ $qrData }}" alt="QR">
                        @endif
                    </div>
